...
int f2[] = {32,2,13,10,12,6}; //add numbers from back of LEDs in animation order

int f3[]= {42,41,40,39,38,37}; //add numbers from back of LEDs in animation order
//
int f4[]= {14,15,16,17,18,19}; //add numbers from back of LEDs in animation order
//
//
...
int f7[] = {1,2,3,4,5,6,7,8}; //add numbers from back of LEDs in animation order

int f8[]= {42,41,40,39,38,37,36,35}; //add numbers from back of LEDs in animation order
//
int f9[]= {32,14,16,18,21,23,15,7}; //add numbers from back of LEDs in animation order
...
if (current == HIGH && previous == LOW && count >=1 && count < 3){
    midfirefly(strip.Color(255, 255, 0), 100);
    Serial.println("MID fireflies"); // When the button is released if the counter is between the
  }                          // two numbers (2-6 blinks or secs) run the program
...
void midfirefly(uint32_t c, uint8_t wait){
//      for(uint32_t i=sine[i]; i<strip.numPixels(); i--){
  for(int i=0; i<8 ; i++){
    strip.setBrightness(8);
//    strip.setPixelColor(f2[i], c);
    strip.setPixelColor(f7[i], c);
//    strip.setPixelColor(f4[i], c);
    strip.setPixelColor(f8[i], c);
    strip.setPixelColor(f9[i], c);
...
//      strip.setPixelColor(f2[i], strip.Color(r, g, b));
      strip.setPixelColor(f7[i], strip.Color(r, g, b));
      strip.setPixelColor(f8[i], strip.Color(r, g, b));
     strip.setPixelColor(f9[i], strip.Color(r, g, b));
      strip.show();
      delay(wait);
    }
    // & fade out in 5 steps
